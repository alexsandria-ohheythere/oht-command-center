...
// Full-time Basic pay is capped at 10 shifts per cutoff (20/month across two cutoffs).
// Any shifts worked beyond that in the same cutoff are still paid — at the same per-day
// rate — but broken out as "Additional Payment" on the payslip instead of being folded
// into Basic, so overtime/extra-shift pay for full-timers is always visible as its own line.
export const FULL_TIME_SHIFTS_PER_CUTOFF = 10
...
// shift definitions (AM 6:30AM–3:30PM, OPS 8:00AM–5:00PM, MID 11AM–8PM, PM 2PM–11PM).
...
// Service Charge eligibility policy (OHT Employee Handbook amendment, effective 2026-09-01):
// a staff member is disqualified for the month if ANY of the following hold —
//   1. Punctuality: at least 3 instances of lateness that month, OR at least 2 instances of
//      lateness of 60+ minutes each (bigLateInstances — see computeCutoffPayroll's bigLateCount,
//      summed across the month's cutoffs).
//   2. Compliance: any recorded violation of company rules/policy that month.
//   3. Advance notice: any unapproved absence — a scheduled shift with no clock-in and no
//      approved leave/day-off on file for that date (see computeMonthNoShowCount in
//      app/payroll/page.js). Advance notice of ≥3 days is expected, except for critical
//      hospitalization, an accident, or the death of an immediate family member — those
//      exceptions are judged by whoever approves the leave/day-off record, not by this function;
//      it only checks whether the absence is *approved*, however it was cleared.
// lateCount < 3 (not ≤3) is a deliberate tightening from the pre-2026-09-01 rule.
export function isServiceChargeEligible(lateCount = 0, violations = 0, unapprovedAbsences = 0, bigLateInstances = 0) {
  return lateCount < 3 && bigLateInstances < 2 && violations === 0 && unapprovedAbsences === 0
}
...
export function computeCutoffPayroll(staff, periodShifts, overrideRates = null, cutoff = null, requiredDays = null) {
  ...
  // Instances of lateness of 60+ minutes — Service Charge policy disqualifies at 2 of these
  // in a month regardless of the total lateCount (see isServiceChargeEligible).
  const bigLateCount = periodShifts.filter(s => (s.lateMinutes || 0) >= 60).length
  ...
  // Full-time: dailyRate is always monthly ÷ 20 — the SAME fixed rate shown in
  //   Settings → Payroll → Rate Cards and on the Staff Profile "Daily rate" field.
  //   It must stay fixed (not diluted/inflated by how many days this particular
  //   cutoff happened to be scheduled for) so the rate printed on every payslip
  //   always matches what's configured in Settings — previously this divided
  //   (monthly ÷ 2) by that cutoff's own scheduled-day count instead, which made
  //   the payslip "Rate" drift cutoff to cutoff and confused staff comparing it
  //   to Settings (e.g. an 11-scheduled-day cutoff showed ₱772.73/day instead of
  //   the configured ₱850/day for a ₱17,000/mo role).
  //   A published schedule is still required before any pay is computed — if
  //   0/unknown scheduled days this cutoff -> no pay (publish roster first).
  ...
  // Basic pay days vs. extra shift days beyond the full-time cap (see FULL_TIME_SHIFTS_PER_CUTOFF).
  // Non-full-time staff have no cap: all days worked count as basic pay days.
  let basicPayDays = daysWorked, extraShiftDays = 0, additionalPayment = 0
  if (type === 'Full-time') {
    const reqd = (requiredDays && requiredDays > 0) ? requiredDays : 0
    if (reqd === 0) {
      noSchedule = true
      dailyRate = 0; hourlyRate = 0; minuteRate = 0; gross = 0
      basicPayDays = 0; extraShiftDays = 0; additionalPayment = 0
    } else {
      dailyRate = monthly / 20
      hourlyRate = dailyRate / MAX_PAID_HOURS_PER_SHIFT
      minuteRate = hourlyRate / 60
      // Cap Basic pay at FULL_TIME_SHIFTS_PER_CUTOFF shifts; anything worked beyond that
      // is still paid (same daily rate) but as a separate Additional Payment line —
      // never silently folded into Basic.
      basicPayDays = Math.min(daysWorked, FULL_TIME_SHIFTS_PER_CUTOFF)
      extraShiftDays = Math.max(0, daysWorked - FULL_TIME_SHIFTS_PER_CUTOFF)
      gross = round2(dailyRate * basicPayDays)
      additionalPayment = round2(dailyRate * extraShiftDays)
    }
  } else {
  ...
  // Additional Payment (extra shifts beyond the full-time cap) is real money owed —
  // it's added to Basic (gross) before deductions, same as any other earnings line.
  const netPay = Math.max(0, gross + additionalPayment - totalDeductions)

  return {
    daysWorked, paidHours: Math.round(paidHours * 100) / 100,
    totalLateMins, lateCount, bigLateCount, gross,
    basicPayDays, extraShiftDays, additionalPayment,
    ...
    // Informational only for this single cutoff — doesn't factor in absences (month-wide) or
    // reflect a manual override; the authoritative monthly check lives in the Service Charge tab.
    eligible: isServiceChargeEligible(lateCount, staff.violation_count || 0, 0, bigLateCount),
  }
}
...
// (isServiceChargeEligible — see its own doc comment for the full policy: fewer
// than 3 late instances and fewer than 2 of 60+ minutes, zero violations, and
// zero unapproved absences for the month, unless an admin has manually
// overridden that staff member's inclusion for the month), proportional to
// each person's paid hours worked in the cutoff. Ineligible
