...
export function computeCutoffPayroll(staff, periodShifts, overrideRates = null, cutoff = null) {
  ...
  const dailyRate = getDailyRate(type, role, overrideRates)
  const hourlyRate = getHourlyRate(type, role, overrideRates)
  const minuteRate = getMinuteRate(type, role, overrideRates)
  ...
  if (type === 'Full-time' && monthly > 0 && isFirstCutoffOfMonth) {
