...
// unapprovedAbsences = no-show days that month with no approved leave/day-off covering them
// (i.e. not cleared in advance — see computeMonthNoShowCount in app/payroll/page.js). Any
// unapproved absence disqualifies outright; approved absences (leave/day-off, including the
// emergency exceptions admins can approve after the fact — bereavement, hospitalization,
// accident, critical illness) never count against eligibility.
export function isServiceChargeEligible(lateCount = 0, violations = 0, unapprovedAbsences = 0) {
  return lateCount <= 3 && violations === 0 && unapprovedAbsences === 0
}
...
// (isServiceChargeEligible — late count ≤3, zero violations, and zero unapproved
// absences for the month, unless an admin has manually overridden that staff
// member's inclusion for the month), proportional to each person's paid hours
// worked in the cutoff. Ineligible
