...
export async function createPurchaseList(supportId, title, requestIds) {
  ...
  const { data: reqItems, error: itemsErr } = await sb()
    .from('purchase_request_items')
    .select(`*, purchase_requests!inner(submitted_by, staff!submitted_by(first_name, last_name))`)
    .in('request_id', requestIds)
  if (itemsErr) throw itemsErr

  if (reqItems?.length > 0) {
    const listItems = reqItems.map(ri => {
      const s = ri.purchase_requests?.staff
      const name = s ? [s.first_name, s.last_name].filter(Boolean).join(' ') : 'Staff'
      return {
        list_id: list.id,
