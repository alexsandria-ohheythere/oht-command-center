...
export default function ExpensesPage() {
  ...
  const [form, setForm] = useState({expense_date:toISO(today),category_id:'',description:'',amount:'',paid_by:'alex',notes:'',receipt_url:''})
  ...
  async function saveExpense() {
    ...
    setSaving(true)
    const {error}=await supabase.from('expenses').insert([{...form,amount:parseFloat(form.amount)||0,category_id:form.category_id||null}])
    if(error){showToast('❌',error.message);setSaving(false);return}
    await fetchAll(); setShowForm(false)
    setForm({expense_date:toISO(today),category_id:'',description:'',amount:'',paid_by:'alex',notes:'',receipt_url:''})
  ...
  const breakdown = categories.map(c=>({
    ...c, total: expenses.filter(e=>e.category_id===c.id).reduce((a,e)=>a+(parseFloat(e.amount)||0),0)
  })).filter(c=>c.total>0).sort((a,b)=>b.total-a.total)

  return (
    <AuthShell>
      <div className="topbar">
        <div><div className="topbar-title">Expenses</div><div className="topbar-sub">{expenses.length} entries · {fmtDate(dateFrom)} – {fmtDate(dateTo)}</div></div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:16,marginBottom:16}}>
                    ...
                    {filtered.map((e,i)=>{
                      ...
                      return(
                        <tr key={e.id} style={{borderBottom:'1px solid var(--border)',background:i%2===0?'var(--white)':'var(--surface)'}}>
                          ...
                          <td style={{padding:'9px 12px',color:'var(--text-muted)',textTransform:'capitalize'}}>{e.paid_by||'—'}</td>
                          <td style={{padding:'9px 12px'}}>{e.receipt_url?<a href={e.receipt_url} target="_blank" rel="noreferrer" style={{fontSize:10,color:'var(--sky)',fontWeight:600,textDecoration:'none'}}>📎 View</a>:<span style={{color:'var(--border)'}}>—</span>}</td>
                          <td style={{padding:'9px 12px'}}><button onClick={()=>deleteExpense(e.id)} style={{background:'transparent',border:'none',color:'var(--border)',cursor:'pointer',fontSize:13}} onMouseEnter={x=>x.target.style.color='#c0392b'} onMouseLeave={x=>x.target.style.color='var(--border)'}>🗑</button></td>
                        </tr>
                      )
                    })}
                  </tbody>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div style={{background:'var(--white)',border:'1px solid var(--border)',borderRadius:13,padding:'18px'}}>
              ...
              <div style={{fontFamily:"'Montserrat',sans-serif",fontSize:28,fontWeight:700,color:'#c0392b',marginBottom:12}}>{peso(total)}</div>
