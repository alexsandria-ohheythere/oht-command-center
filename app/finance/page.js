...
const TABS = [
  ...
  { id:'products',   label:'Product Performance',  icon:'🛍️' },
    ...
    reader.onload = async ev => {
      ...
      for (let i=1;i<lines.length;i++) {
        ...
        if (breakdownType==='category') {
          ...
          if (!base.category && base.gross_sales===0 && base.net_sales===0) continue
        ...
        {tab==='products'&&(()=>{
          ...
          <div>
            <div style={{display:'flex',gap:9,marginBottom:10,flexWrap:'wrap',alignItems:'center'}}>
              <label style={{display:'flex',alignItems:'center',gap:6,background:'var(--sky-pale)',border:'1px solid var(--sky)',borderRadius:8,padding:'7px 14px',fontSize:11,fontWeight:700,color:'var(--sky)',cursor:'pointer'}}>
                📂 Upload StoreHub Report CSV
                <input type="file" accept=".csv" ref={productFileRef} style={{display:'none'}} onChange={handleProductCSV}/>
              </label>
              <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:14}}>
                ...
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:12,color:'var(--text-muted)'}}>{rows.length} rows</div>
              </div>
            </div>
