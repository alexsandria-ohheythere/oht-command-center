...
const EMPTY = {first_name:'',last_name:'',nickname:'',role:'Senior Barista',employment_type:'Full-time',email:'',phone:''}
...
export default function StaffPage() {
  ...
  function startEdit(s) {
    setEditingId(s.id)
    setEditForm({ first_name:s.first_name||'', last_name:s.last_name||'', nickname:s.nickname||'', role:s.role||'Senior Barista', employment_type:s.employment_type||'Full-time', email:s.email||'', phone:s.phone||'' })
  }
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
              <thead>
                <tr style={{background:'var(--espresso)'}}>
                  {['','Name','Role','Type','Email','Phone','Actions'].map(h=>(
