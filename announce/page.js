...
// ── Send Messenger notification to a single staff member ────────────────────
async function sendMessenger(staffId, message) {
  try {
  ...
  } catch(e) { console.warn('Messenger notify failed:', e) }
}
...
export default function AnnouncementsPage() {
  ...
  async function fetchAll() {
    ...
    const [{ data: a }, { data: s }] = await Promise.all([
      ...
      supabase.from('staff').select('id,first_name,last_name,nickname,messenger_psid,messenger_opted_in').order('last_name'),
    ])
    setAnnouncements(a || [])
    setStaff(s || [])
    setLoading(false)
  ...
  async function postAnnouncement() {
    ...
    // ── Messenger notifications (fire for all opted-in staff in target) ──
    const linkedStaff = targetStaff.filter(s => s.messenger_opted_in && s.messenger_psid)
    const messengerMsg = `📣 OHT Announcement\n\n${form.title}\n\n${form.content}\n\n— ${form.posted_by}`
    await Promise.allSettled(linkedStaff.map(s => sendMessenger(s.id, messengerMsg)))

    await fetchAll()
    setShowForm(false)
    setForm({ title:'', content:'', posted_by:'Alex', is_pinned:false, notify_all:true, selected_staff:[] })
    showToast('📣', `Announcement posted · ${targetStaff.length} notified${linkedStaff.length > 0 ? ` · ${linkedStaff.length} via Messenger` : ''}`)
    setSaving(false)
  }
    ...
    <AuthShell>
      <div className="topbar">
        <div>
          ...
          <div className="topbar-sub">{announcements.length} posts · staff notified via portal & Messenger</div>
        </div>
